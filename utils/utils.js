import { getCustomConfig } from '../scripts/scripts.js';
...
export function localizeLink(href, originHostName = window.location.hostname) {
  try {
    ...
    const { locale, locales, prodDomains } = getCustomConfig();
...
export function appendHtmlPostfix(area = document) {
  ...
  const { autoBlocks = [], htmlExclude = [] } = getCustomConfig();
...
export async function loadTemplate() {
  ...
  const { miloLibs, codeRoot } = getCustomConfig();
...
export function loadPrivacy() {
  ...
  if (getCustomConfig().env.name === 'stage') {
    domainId += '-test';
  }
  window.fedsConfig = {
    privacy: {
...
function decorateMeta() {
  ...
  window.addEventListener('modal:open', async (e) => {
    const { miloLibs } = getCustomConfig();
...
export async function loadArea(area = document) {
  ...
  if (isDoc) {
    ...
    loadFavIcon(createTag, getCustomConfig(), getMetadata);
    initSidekick();
  }
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      if (getMetadata('interlinks') === 'on') {
        const path = `${getCustomConfig().locale.contentRoot}/keywords.json`;
