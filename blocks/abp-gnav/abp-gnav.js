...
class Gnav {
  ...
  loadSearch = async () => {
    ...
    if (this.searchType === SEARCH_TYPE_CONTEXTUAL) {
      const { default: onContextualSearchInput } = await import('./abp-gnav-contextual-search.js');
...
export default async function init(header) {
  const { locale, imsClientId } = getCustomConfig();
